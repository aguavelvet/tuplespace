...
class TupleSpace:
    ...
    def register (self, id:str, state:str, host:str, port:str, endpoint:str):
        if state not in self.registry:
            self.registry[state] = []

        self.registry[state].append ({ "id" : id, "state" : state, "host" : host, "port": int(port), "endpoint":endpoint })
    ...
    def reregister (self, current_state:str, new_req_state:str, new_post_state:str):
        ...
        if new_req_state not in self.registery:
            self.registry[new_req_state] = workers
        else:
            self.registry[new_req_state].extend(workers)
